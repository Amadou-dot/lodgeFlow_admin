...
- Reuse `lodgeflow.app`, which the user confirms is already configured for sending
  in Resend. No new domain verification or DNS setup is planned. Confirm the
  intended mailbox/display name from the existing configuration during this slice;
  the domain confirmation does not specify a sender address.
...
- [ ] Builds still work without Stripe/Resend secrets.
