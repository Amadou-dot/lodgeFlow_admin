...
| Customer `POST /api/bookings`                 | Matches the full populated JSON envelope; creates one booking for the verified customer, calculates total 300 and deposit due 75 from server data, stores no receipts. Client-supplied customer/price cannot override these.                      | Missing/expired/tampered sessions redirect to the expected Clerk sign-in endpoint and create nothing; invalid guest count returns 400 with no write; overlap returns 409 without a second booking.                                  |
| Customer `GET /api/bookings/[id]`             | Own booking matches full hydrated JSON, including IDs, ISO dates and virtuals; a deleted cabin remains null.                                                                                                                                                 | Foreign/missing bookings return safe 404; missing auth is rejected; database failures return safe 500 without database/provider effects.                                                                                                                                                                                  |
| Customer `GET /api/bookings/history` | Owner rows match the lean JSON projection; status filters retain matching rows; deleted cabins remain null. | Missing auth is rejected, foreign history is empty, database failures return safe 500 without writes/provider calls. |
| Customer `PATCH /api/bookings/[id]`           | Allowed guest update persists with the full populated JSON envelope; injected price and unsupported dates/notes are ignored.                                                                                                                                         | Foreign/missing/invalid IDs return 404; unauthenticated requests are rejected; invalid guest input/capacity returns 400; injected database write failure returns safe 500. Failures preserve the complete row and make no provider call.                                                                                                                                                                 |
...
| Customer `GET /api/bookings/[id]/refund-estimate` | Received-money estimate and ISO/null deadlines match domain results; terminal states and active checkout report cancellation unavailable. | Missing authentication is rejected; foreign and missing bookings return identical 404 without writes/provider calls. |
| Customer `DELETE /api/bookings/[id]`          | Returns the full serialized booking on initial cancellation and retry; cancels and persists refund intent capped at received money (75, not the 300 booking total); recovered provider retry uses the same idempotency key; repeat cancellation does not issue another refund. | Foreign cancellation returns 404 without writes or refund calls. Provider failure preserves cancellation plus explicitly pending refund/error in the existing 200 envelope; completed refund remains zero until the signed webhook. |
...
- **Phase 1 cabin boundary slice:** read serialization, missing populated cabins, status filtering, ownership and database-error coverage now accompany the first DTO slice. Expand mutation methods' remaining invalid-ID, missing-resource, database-error and auth permutations before changing those contracts. Existing domain
